...
function getFinDateRange(preset) {
  ...
  let start;
  if (preset === 'thisweek') {
    const d = new Date(now); d.setDate(d.getDate() - 7); start = toLocalDateStr(d);
  } else if (preset === 'thismonth') {
    start = toLocalDateStr(new Date(now.getFullYear(), now.getMonth(), 1));
  } else if (preset === '1month') {
    const d = new Date(now); d.setDate(d.getDate() - 30); start = toLocalDateStr(d);
  } else if (preset === '3months') {
    const d = new Date(now); d.setDate(d.getDate() - 90); start = toLocalDateStr(d);
  } else if (preset === '6months') {
    const d = new Date(now); d.setDate(d.getDate() - 180); start = toLocalDateStr(d);
  } else {
    start = toLocalDateStr(new Date(now.getFullYear(), now.getMonth(), 1));
  }
...
export default function ReportsPage() {
  ...
  const [expForm, setExpForm] = useState({ amount: '', date: toLocalDateStr(new Date()), mode: 'cash', remarks: '' });
  ...
  const fetchComparison = async () => {
    setLoading(true);
    try {
      const params = compMode === 'custom' && compP1Start && compP1End && compP2Start && compP2End
        ? { p1Start: compP1Start, p1End: compP1End, p2Start: compP2Start, p2End: compP2End }
        : {};
  ...
  const handleAddExpense = async (e) => {
    e.preventDefault();
    if (!expForm.amount || parseFloat(expForm.amount) <= 0) { toast.error('Enter a valid amount'); return; }
    if (!expForm.date) { toast.error('Select a date'); return; }
    ...
    try {
      ...
      setExpForm({ amount: '', date: toLocalDateStr(new Date()), mode: 'cash', remarks: '' });
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-2xl border border-primary-100 p-4 flex flex-wrap gap-3 items-center justify-between">
            <div className="flex flex-wrap gap-2">
              {FIN_PRESETS.map(p => (
                <button key={p.value} onClick={() => setFinPreset(p.value)}
            ...
            {showAddExpense && (
              ...
              >
                <div className="flex items-center justify-between mb-4">
                  ...
                  <button onClick={() => setShowAddExpense(false)} className="text-primary-lighter hover:text-primary transition-colors">
                    <X size={18} />
                  </button>
                </div>
                <form onSubmit={handleAddExpense} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                  <div>
                    <label className="block text-xs font-medium text-primary-lighter mb-1.5">Amount (₹)</label>
                    <input
                      type="number"
                      min="1"
                      step="0.01"
                  ...
                  <div>
                    <label className="block text-xs font-medium text-primary-lighter mb-1.5">Date</label>
                    <input
                      type="date"
                      value={expForm.date}
                  ...
                  <div>
                    <label className="block text-xs font-medium text-primary-lighter mb-1.5">Mode</label>
                    <select
                      value={expForm.mode}
                  ...
                  <div>
                    <label className="block text-xs font-medium text-primary-lighter mb-1.5">Remarks</label>
                    <input
                      type="text"
          ...
          {finLoading ? (
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {[...Array(3)].map((_, i) => <div key={i} className="h-24 bg-white rounded-2xl border border-primary-100 animate-pulse" />)}
            </div>
          ) : financialsData && (
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="bg-white rounded-2xl border border-green-200 p-5">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-xs font-medium text-primary-lighter uppercase tracking-wide">Total Earnings</span>
                </div>
                <div className="text-2xl font-display font-bold text-green-700">{formatCurrency(financialsData.summary.totalEarnings)}</div>
              </div>
              <div className="bg-white rounded-2xl border border-red-200 p-5">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-xs font-medium text-primary-lighter uppercase tracking-wide">Total Expenses</span>
                </div>
                <div className="text-2xl font-display font-bold text-red-700">{formatCurrency(financialsData.summary.totalExpenses)}</div>
              </div>
              <div className={`rounded-2xl border p-5 ${financialsData.summary.netProfit >= 0 ? 'bg-white border-primary-100' : 'bg-red-50 border-red-200'}`}>
                <div className="flex items-center gap-2 mb-2">
                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${financialsData.summary.netProfit >= 0 ? 'bg-primary-100' : 'bg-red-100'}`}>
                    <IndianRupee size={16} className={financialsData.summary.netProfit >= 0 ? 'text-primary' : 'text-red-600'} />
                  </div>
                  <span className="text-xs font-medium text-primary-lighter uppercase tracking-wide">Net Profit</span>
                </div>
                <div className={`text-2xl font-display font-bold ${financialsData.summary.netProfit >= 0 ? 'text-primary' : 'text-red-700'}`}>
                  {financialsData.summary.netProfit < 0 ? '-' : ''}{formatCurrency(Math.abs(financialsData.summary.netProfit))}
                </div>
              </div>
            </div>
          )}
          ...
          {!finLoading && financialsData?.monthlyData?.length > 0 && (
            <div className="bg-white rounded-2xl border border-primary-100 p-5">
              <h2 className="font-semibold text-primary mb-5">Monthly Earnings vs Expenses</h2>
              <ResponsiveContainer width="100%" height={300}>
                <BarChart data={financialsData.monthlyData} margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#f5e8e0" />
                  <XAxis dataKey="month" tick={{ fontSize: 11, fill: CHART_COLORS.light }} />
                  <YAxis tick={{ fontSize: 11, fill: CHART_COLORS.light }} tickFormatter={v => `₹${v >= 1000 ? (v/1000).toFixed(0)+'k' : v}`} />
                  <Tooltip content={<CustomTooltip />} />
                  <Legend />
                  <Bar dataKey="earnings" name="Earnings" fill={CHART_COLORS.green} radius={[4,4,0,0]} />
                  <Bar dataKey="expenses" name="Expenses" fill={CHART_COLORS.red} radius={[4,4,0,0]} />
                  <Bar dataKey="profit" name="Net Profit" fill={CHART_COLORS.gold} radius={[4,4,0,0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          )}
          ...
          {!finLoading && financialsData?.dailyData?.length > 0 && (
            <div className="bg-white rounded-2xl border border-primary-100 p-5">
              <h2 className="font-semibold text-primary mb-5">Daily Activity</h2>
              <ResponsiveContainer width="100%" height={260}>
                <AreaChart data={financialsData.dailyData} margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
                  <defs>
                    <linearGradient id="finEarnings" x1="0" y1="0" x2="0" y2="1">
                  ...
                  <CartesianGrid strokeDasharray="3 3" stroke="#f5e8e0" />
                  <XAxis dataKey="date" tick={{ fontSize: 10, fill: CHART_COLORS.light }} />
                  <YAxis tick={{ fontSize: 11, fill: CHART_COLORS.light }} tickFormatter={v => `₹${v >= 1000 ? (v/1000).toFixed(0)+'k' : v}`} />
                  <Tooltip content={<CustomTooltip />} />
                  <Legend />
                  <Area type="monotone" dataKey="earnings" name="Earnings" stroke={CHART_COLORS.green} fill="url(#finEarnings)" strokeWidth={2} />
                  <Area type="monotone" dataKey="expenses" name="Expenses" stroke={CHART_COLORS.red} fill="url(#finExpenses)" strokeWidth={2} />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          )}
          ...
          {!finLoading && financialsData && (
            <div className="bg-white rounded-2xl border border-primary-100 overflow-hidden">
              <div className="p-5 border-b border-primary-50 flex items-center justify-between">
                ...
                <span className="text-xs text-primary-lighter">{financialsData.expenses?.length || 0} entries</span>
              </div>
              {!financialsData.expenses?.length ? (
                <div className="py-12 text-center text-primary-lighter text-sm">No expenses recorded for this period</div>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-primary-50">
                        <th className="px-5 py-3 text-left text-xs font-medium text-primary-lighter uppercase tracking-wide">Date</th>
                        <th className="px-5 py-3 text-left text-xs font-medium text-primary-lighter uppercase tracking-wide">Amount</th>
                        <th className="px-5 py-3 text-left text-xs font-medium text-primary-lighter uppercase tracking-wide">Mode</th>
                        <th className="px-5 py-3 text-left text-xs font-medium text-primary-lighter uppercase tracking-wide">Remarks</th>
                        <th className="px-5 py-3 text-left text-xs font-medium text-primary-lighter uppercase tracking-wide">Added By</th>
                        <th className="px-2 py-3" />
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-primary-50">
                      {financialsData.expenses.map(exp => (
                        <tr key={exp._id} className="hover:bg-primary-50/40 transition-colors group">
                          <td className="px-5 py-3.5 text-primary whitespace-nowrap">{formatDate(exp.date)}</td>
                          <td className="px-5 py-3.5 font-semibold text-primary whitespace-nowrap">{formatCurrency(exp.amount)}</td>
                          <td className="px-5 py-3.5">
                            <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-medium ${exp.mode === 'cash' ? 'bg-amber-100 text-amber-700' : 'bg-blue-100 text-blue-700'}`}>
                              {exp.mode === 'cash' ? <Banknote size={11} /> : <CreditCard size={11} />}
        ...
        <div className="space-y-6">
          ...
          {loading && (
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
              {[...Array(3)].map((_, i) => <div key={i} className="h-28 bg-white rounded-2xl border border-primary-100 animate-pulse" />)}
            </div>
          )}
