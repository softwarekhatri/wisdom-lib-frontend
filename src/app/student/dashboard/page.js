...
import {
  ...
  IndianRupee, ChevronLeft, ChevronRight, Sparkles, Clock,
  AlertCircle, CheckCircle, Lock, Eye, EyeOff, Shield
} from 'lucide-react';
import toast from 'react-hot-toast';

/* ─── Confetti ──────────────────────────────────────────────────── */
const CONFETTI_COLORS = ['#c9a15e', '#43332c', '#e8c98a', '#6b5548', '#fff', '#f59e0b'];
function Confetti() {
  return (
    <div className="absolute inset-0 pointer-events-none overflow-hidden z-0">
      {Array.from({ length: 50 }).map((_, i) => (
        <motion.div
          ...
          animate={{ y: ['0vh', '110vh'], x: [(Math.random() - 0.5) * 60, (Math.random() - 0.5) * 120], rotate: [0, Math.random() * 720], opacity: [1, 1, 0] }}
          transition={{ duration: Math.random() * 3 + 2, delay: Math.random() * 3, ease: 'linear', repeat: Infinity, repeatDelay: Math.random() * 4 }}
        />
      ))}
    </div>
  );
    ...
    >
      ...
      <div className="relative z-10 text-center">
        ...
        <h2 className="font-display text-2xl sm:text-3xl font-bold mb-2 drop-shadow">
          Happy {yearsCompleted} Year{yearsCompleted > 1 ? 's' : ''} Anniversary!
        </h2>
        <p className="text-white/75 text-sm max-w-md mx-auto">
          Congratulations on completing {yearsCompleted} wonderful year{yearsCompleted > 1 ? 's' : ''} as a
          proud member of the Wisdom Library family!
        </p>
        <div className="flex items-center justify-center gap-1.5 mt-4">
          {Array.from({ length: Math.min(yearsCompleted, 5) }).map((_, i) => (
            <motion.div key={i} animate={{ scale: [1, 1.4, 1] }} transition={{ delay: i * 0.15, duration: 0.5, repeat: Infinity, repeatDelay: 3 }}>
...
function MembershipCard({ student, membershipDuration }) {
  const joinYear = student?.admissionDate ? new Date(student.admissionDate).getFullYear() : '—';
  const memberId = student?._id ? `WL-${String(student._id).slice(-6).toUpperCase()}` : 'WL-000000';
  ...
  return (
    <motion.div
      ...
      style={{ background: 'linear-gradient(135deg, #1a0f0a 0%, #2d2219 40%, #43332c 70%, #5a4038 100%)' }}
    >
      {/* Shimmer overlay */}
      <div
        className="absolute inset-0 opacity-20"
        style={{
          background: 'linear-gradient(105deg, transparent 40%, rgba(201,161,94,0.4) 50%, transparent 60%)',
      ...
      <div className="relative z-10 p-6 sm:p-8">
        ...
        <div className="flex items-start justify-between mb-6">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <div className="text-gold font-display font-bold text-sm tracking-wide">WISDOM LIBRARY</div>
              <div className="text-white/40 text-xs tracking-widest uppercase">Membership Card</div>
            </div>
          </div>
          <div className="text-right">
            <div className="text-gold/60 text-xs tracking-widest uppercase mb-0.5">Member ID</div>
            <div className="text-white/80 font-mono text-sm font-semibold">{memberId}</div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-5 mb-6">
          ...
          <div>
            <div className="text-white/60 text-xs uppercase tracking-widest mb-0.5">Card Holder</div>
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            {
              label: 'Membership',
              value: membershipDuration?.years > 0
                ? `${membershipDuration.years}y ${membershipDuration.months}m`
                : `${membershipDuration?.totalMonths || 0} months`,
            },
            { label: 'Monthly Fee', value: formatCurrency(student?.libraryFees) },
            { label: 'Member Since', value: formatDate(student?.admissionDate, 'MMM dd, yyyy') },
          ].map(({ label, value }) => (
            <div key={label} className="bg-white/5 rounded-xl px-3 py-2.5 border border-white/8" style={{ borderColor: 'rgba(255,255,255,0.08)' }}>
              <div className="text-gold/60 text-xs uppercase tracking-wide mb-0.5">{label}</div>
...
function PaymentStatusCard({ payStatus, libraryFees }) {
  ...
  const outstandingMonths = isOverdue ? Math.max(1, Math.ceil(-payStatus.daysUntilDue / 30)) : 0;
        ...
        <div className="flex-1 min-w-0">
          <div className="flex flex-wrap items-center justify-between gap-2 mb-2">
            <h3 className="text-white font-display font-bold text-lg leading-tight">
              {isOverdue ? 'Payment Overdue!' : isDueSoon ? 'Payment Due Soon' : 'Payment Up to Date'}
            </h3>
            <span className={`text-xs font-bold px-3 py-1 rounded-full ${isOverdue ? 'bg-red-500/40 text-red-200' : isDueSoon ? 'bg-orange-500/40 text-orange-200' : 'bg-emerald-500/40 text-emerald-200'}`}>
              {isOverdue ? 'ACTION NEEDED' : isDueSoon ? `${payStatus.daysUntilDue} DAYS LEFT` : 'ALL CLEAR'}
            </span>
          </div>
          ...
          <div className="space-y-1.5 text-sm">
            ...
            <div className="flex items-center gap-2 text-white/80">
              <span className="text-white/50 w-28 flex-shrink-0">Next Due Date:</span>
              <span className={`font-bold ${isOverdue ? 'text-red-300' : isDueSoon ? 'text-orange-300' : 'text-white'}`}>
                {payStatus.dueDateLabel}
              </span>
            </div>
            {libraryFees > 0 && (
              <div className="flex items-center gap-2 text-white/80">
                <span className="text-white/50 w-28 flex-shrink-0">Amount Due:</span>
                <span className="font-bold text-gold">{formatCurrency(libraryFees)}</span>
              </div>
            )}
          </div>
          ...
          {(isOverdue || isDueSoon) && (
            <div className="mt-3 pt-3 border-t border-white/10 text-xs text-white/60 flex items-center gap-1.5">
              <span>📍</span>
              <span>Payments accepted at the library office only &nbsp;·&nbsp; +91 98765 43210</span>
            </div>
          )}
        </div>
      </div>
...
function QuickStats({ membershipDuration, totalPaid, payStatus, libraryFees }) {
  const stats = [
    ...
    },
    {
      ...
      sub: payStatus.daysUntilDue >= 0
        ? `in ${payStatus.daysUntilDue} days`
        : `${Math.abs(payStatus.daysUntilDue)} days overdue`,
      color: payStatus.status === 'paid' ? 'from-emerald-600 to-green-500' : payStatus.status === 'due-soon' ? 'from-orange-600 to-orange-400' : 'from-red-700 to-red-500',
    },
    {
      icon: '🏷️',
      label: 'Monthly Fee',
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
      {stats.map((s, i) => (
        ...
        >
          <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${s.color} flex items-center justify-center text-lg mb-3 group-hover:scale-110 transition-transform`}>
            {s.icon}
          </div>
          <div className="text-xs text-primary-lighter font-medium uppercase tracking-wide mb-0.5">{s.label}</div>
          <div className="text-primary font-display font-bold text-base leading-tight">{s.value}</div>
      ...
      >
        <div className="bg-gradient-to-r from-primary to-primary-light p-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-white font-display font-bold text-lg">Change Password</h2>
              <p className="text-white/60 text-xs">Keep your account secure</p>
            </div>
          </div>
        </div>
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <Field id="currentPassword" label="Current Password" value={form.currentPassword} showKey="current" />
          <Field id="newPassword" label="New Password" value={form.newPassword} showKey="new" />
          <Field id="confirmPassword" label="Confirm New Password" value={form.confirmPassword} showKey="confirm" />
          <div className="flex gap-3 pt-2">
            <button type="button" onClick={onClose} className="flex-1 px-4 py-3 rounded-xl border border-primary-200 text-primary text-sm font-medium hover:bg-primary-50">Cancel</button>
            <button type="submit" disabled={loading} className="flex-1 btn-primary flex items-center justify-center gap-2 text-sm">
  ...
  return (
    ...
    >
      <div className="absolute inset-0 opacity-5"
        style={{ backgroundImage: 'radial-gradient(rgba(201,161,94,0.6) 1px, transparent 1px)', backgroundSize: '20px 20px' }} />
      <div className="relative z-10 flex flex-col sm:flex-row items-center gap-4 text-center sm:text-left">
        ...
        <div>
          ...
          <p className="text-white/60 text-sm">
            Continuously growing since {formatDate(admissionDate, 'MMMM d, yyyy')}
            {membershipDuration?.years >= 1 && (
              <span className="text-gold ml-1 font-semibold">
                — {membershipDuration.years} year{membershipDuration.years > 1 ? 's' : ''} of excellence!
              </span>
            )}
          </p>
          {membershipDuration?.years >= 1 && (
            <div className="flex items-center gap-1 mt-2 justify-center sm:justify-start">
              {Array.from({ length: Math.min(membershipDuration.years, 5) }).map((_, i) => (
                <Trophy key={i} className="w-4 h-4 text-gold fill-gold/30" />
              ))}
              <span className="text-gold text-xs font-bold ml-1">{membershipDuration.years}x Year Achiever</span>
            </div>
          )}
        </div>
      </div>
    ...
    >
          ...
          {payments.map((p, i) => (
            ...
            >
              ...
              <div className="flex flex-col items-center flex-shrink-0">
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center shadow-sm ${p.mode === 'cash' ? 'bg-green-100' : 'bg-blue-100'}`}>
                  {p.mode === 'cash' ? (
                    <IndianRupee className="w-4.5 h-4.5 text-green-600" style={{ width: 18, height: 18 }} />
                  ) : (
                    <CreditCard className="w-4.5 h-4.5 text-blue-600" style={{ width: 18, height: 18 }} />
                  )}
                </div>
              </div>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <p className="text-sm font-semibold text-primary">{formatDate(p.receivedDate)}</p>
                    ...
                    {p.referenceNo && (
                      <p className="text-xs text-primary-lighter/70 mt-0.5 font-mono">Ref: {p.referenceNo}</p>
                    )}
                  </div>
                  <div className="text-right flex-shrink-0">
                    <p className="text-base font-display font-bold text-primary">{formatCurrency(p.amount)}</p>
                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${p.mode === 'cash' ? 'bg-green-100 text-green-700' : 'bg-blue-100 text-blue-700'}`}>
      ...
      {pagination && pagination.pages > 1 && (
        <div className="flex items-center justify-center gap-3 p-4 border-t border-primary-50 bg-primary-50/30">
          ...
          <span className="text-xs text-primary-lighter font-medium">Page {page} of {pagination.pages}</span>
...
export default function StudentDashboard() {
    ...
    return (
      <div className="space-y-4 animate-pulse">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[...Array(4)].map((_, i) => <div key={i} className="h-28 bg-white rounded-2xl border border-primary-100" />)}
        </div>
  ...
  const { isAnniversary, yearsCompleted } = isAnniversaryWindow(student?.admissionDate);

  const totalMonthsPaid = allPayments.reduce((sum, p) => sum + (p.monthsCovered?.length || 0), 0);
  ...
  return (
    <div>
      ...
      {/* Membership card */}
      <MembershipCard student={student} membershipDuration={membershipDuration} />

      {/* Payment status */}
      <PaymentStatusCard payStatus={payStatus} libraryFees={student?.libraryFees} />
      ...
      {/* Tagline */}
      <TaglineSection membershipDuration={membershipDuration} admissionDate={student?.admissionDate} />

      {/* Payment history */}
      <PaymentHistory payments={payments} page={page} setPage={setPage} pagination={pagination} />
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-primary-lighter">Update your login password</p>
          </div>
        </div>
        <button
